...
public class EntityCount extends Metric<AtomicInteger> {
    ...
    public EntityCount(String name, MetricsManager manager) {
        super(name, "gauge", manager, new AtomicInteger());
    }

    @Override
    public void run() {
        this.collector.set(this.manager.getEntityCount());
    }
    ...
    public Component formatComponent() {
        return text().append(text("Entities: ", GOLD)).append(text(String.format("%d", this.collector.intValue()), WHITE)).asComponent();
    }
